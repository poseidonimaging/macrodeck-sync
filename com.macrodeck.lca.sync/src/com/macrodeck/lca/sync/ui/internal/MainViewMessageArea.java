 ...
 * @since 3.2
 */
class MainViewMessageArea extends DialogMessageArea {
	...
	private MainView preferenceDialog;
	 ...
	 *            The PreferenceDialog that creates the message area.
	 */
	public MainViewMessageArea(MainView preferenceDialog) {
	...
	public void createContents(Composite parent) {
		...
		messageArea = new ImageAndMessageArea(
				preferenceDialog.formTitleComposite, SWT.WRAP);
	...
	public void setMessageLayoutData(Object layoutData) {
		...
		((FormData) layoutData).right = new FormAttachment(100,
				-getTitleLabelOffset());
		messageArea.setLayoutData(layoutData);
	}

	/**
	 * Calculates the width difference between the title label and the parent
	 * composite of the message area. Required for when the workbench adds a
	 * toolbar to the preference dialog.
	 * 
	 * @return the width difference if the calculation yields a value > 0,
	 *         otherwise it will return 0.Subsequent calls will immediately
	 *         return the width difference if it has already been calculated and
	 *         is > 0.
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.eclipse.jface.dialogs.DialogMessageArea#updateText(java.lang.String,
	 *      int)
	 */
	public void updateText(String newMessage, int newType) {
		...
		if (!messageArea.isVisible() && messageArea.getBounds().y != bottom) {
			messageArea.setBounds(messageArea.getBounds().x, bottom, titleLabel
					.getBounds().width, messageArea.computeSize(SWT.DEFAULT,
					SWT.DEFAULT).y);
		}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.eclipse.jface.dialogs.DialogMessageArea#showTitle(java.lang.String,
